...
export const Header = ({ onOpenGroupModal }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-slate-800/80 bg-slate-950/90 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 sm:py-0 sm:h-16 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-4">
        
        {/* Row 1 on Mobile / Left on Desktop: Brand Logo + User Actions on Mobile */}
        <div className="flex items-center justify-between sm:justify-start gap-3 w-full sm:w-auto">
          {/* Brand Logo & Title */}
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-brand-600 to-brand-400 flex items-center justify-center shadow-lg shadow-brand-500/20 shrink-0">
              <CreditCard className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="text-base sm:text-lg font-extrabold text-white tracking-tight flex items-center gap-1.5">
                GastoCompartido
                <span className="px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider rounded-md bg-brand-500/20 text-brand-300 border border-brand-500/30">
                  Cuotas
                </span>
              </h1>
        ...
        {/* Row 2 on Mobile / Center on Desktop: Group Selector & Invite Code */}
        {user && (
          <div className="flex items-center justify-between sm:justify-center gap-2 w-full sm:w-auto border-t sm:border-t-0 border-slate-800/60 pt-2 sm:pt-0">
            ...
            {activeGroup ? (
              <div className="relative">
                ...
                >
                  ...
                  <span className="max-w-[140px] sm:max-w-[180px] truncate">{activeGroup.name}</span>
            ...
            {activeGroup?.invite_code && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900/90 border border-slate-800 hover:border-slate-700 text-slate-300 text-xs font-mono transition-all"
              >
        ...
        {/* Right on Desktop: User Avatar & Logout (Hidden on mobile) */}
        {user && (
